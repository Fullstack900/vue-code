...
this.attendanceService
      ...
      .subscribe({
        next: (res: any) => {
          if (res.statusCode == 200) {
            ...
            isHr ? this.gethrLRMetaData(rowData) :this.getEmployeeLRData(rowData);
          }
        },
        error: (error) => {
      ...
      .subscribe({
        next: (res: any) => {
          if (res.statusCode == 200) {
            ...
            data.requested_shift_string=employeeData.requested_shift_name;
            data.current_shift_string=employeeData.current_shift_name;
  ...
  onSelectLeaveType(event) {
    ...
    if (this.componentType.type == "Emp-form") {
      ...
      let date = this.sharedService.numberFormater(fromDate.getDate() + 2);
      let month = this.sharedService.numberFormater(fromDate.getMonth() + 1);
      let fromDateformated = `${year}-${month}-${date}`;
      //to patch the exact day on date picker increse the day by on
      let toDate = new Date(leaveType.to_date);
      let toYear = toDate.getFullYear();
      let toDay = this.sharedService.numberFormater(toDate.getDate() + 2);
      let toMonth = this.sharedService.numberFormater(toDate.getMonth() + 1);
  ...
  getAllShiftsTypes() {
    // if (this.attendCorreMData == undefined) {
    this.attendanceService
      .getShiftMData(`${APIs.getEmpShiftTypes}?employee_id=${this.employeeId}`)
      .subscribe({
        next: (res: any) => {
          if (res.status == "pass") {
  ...
  //to handle the click on shift dropdown
  ...
  //to set the shift name and value
  reqShiftclick(shift) {
    this.shiftChangeRequestForm.controls["requested_shift"].setValue(
      shift.shift_id
    );
  ...
  //to  sort data in LM.HR order with count
